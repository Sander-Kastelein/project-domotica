...
#include <RCSwitch.h>
   ...
   bool status1 = false;  // Boolean that keeps track of the status of the first rf switch
   bool status2 = false;  // Boolean that keeps track of the status of the second rf switch
   bool status3 = false;  // Boolean that keeps track of the status of the third rf switch
...
bool changed         = true;             // Boolean used later to stop spamming the rf-signal once the desired action has already taken place.        
void setup()
{
	 //Init I/O-pins
...
void loop()
{
   ...
   sendRF(2379297); // used to send a couple of rf signals to turn all switches off when a connection to the arduino server is established.

      
   // Do what needs to be done while the socket is connected.
	 while (ethernetClient.connected()) 
	 {
      checkEvent(switchPin, pinState);                                          // this will continuously keep track of the value the lightsensor is reading while the ethernet client is connected
      sensorValue = readSensor(0, 100); 
      Serial.print("LightSensor Value: "); Serial.println(sensorValue); 

      sensorValue2 = readSensor(1, 100);                                        // this will continuously keep track of the value the temperaturesensor is reading while the ethernet client is connected
      Serial.print("TemperatureSensor Value: "); Serial.println(sensorValue2);  

      
      if(sensorValue <= thresholdValue) { // Sends a rf-signal to turn a switch off once the lightsensor reads a value lower than an established threshold.
         if(!changed){ 
          changed = true;                 // Changes this Boolean so it will now look to see when to use the if loop for the other Threshold
          sendRF(2379310);                // Sends RF-signals to turn the switch off.
          status1 = !status1;             // Changes the status of the same switch.
                         
         }
         
      }
      if(sensorValue >= thresholdValue2){ // Sends a rf-signal to turn a switch on once the lightsensor reads a value higher than an established threshold
        if(changed){
         changed = false;                 // Changes this Boolean so it will now look to see when to use the if loop for the other Threshold
         sendRF(2379311);                 // Sends RF-signals to turn the switch on
         status1 = !status1;              // Changes the status of the same switch
